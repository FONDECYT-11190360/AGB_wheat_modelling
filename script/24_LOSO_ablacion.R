...
metrics_per_fold |>
  ...
  mutate(
    recipe = factor(recipe,
                    ...
                    labels = c("S2+Weather", "S2+S1+Weather", "Fusión (all)"))
  ) |>
  ...
  labs(y = "Value", x = "Combination of sensors") +
...
metrics_per_fold |>
  ...
  mutate(
    recipe = factor(recipe,
                    levels = c("baseline", "radar", "fusion"),
                    labels = c("S2+Weather", "S2+S1+Weather", "Fusion"))
  ) |>
  ...
  labs(x = "Combination of sensors",
       y = "Site-Season (validation's fold)") +
  theme_bw() +
